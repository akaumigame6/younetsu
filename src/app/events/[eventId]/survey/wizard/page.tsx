...
import type { FeedbackData, SurveyData, Exhibit } from '../../../../../types';
import { supabase } from '../../../../../lib/supabase';
import { getExistingEventFeedback, getExistingExhibitFeedback, saveEventFeedback, saveExhibitFeedback } from '../../../../../actions/survey';
...
function SurveyWizardContent() {
  ...
  useEffect(() => {
    if (exhibitId && eventId) {
      fetch(`/api/exhibits?eventId=${eventId}`)
        .then(res => res.json())
        .then(data => {
           const found = data.find((c: Exhibit) => c.id === exhibitId);
           if (found) setExhibit(found);
        })
        .catch(console.error);
    }
  }, [exhibitId, eventId]);
  ...
  // Custom Answersの初期化は既存データやinitialDataから行う
  useEffect(() => {
    if (initialData?.customAnswers && Object.keys(customAnswers).length === 0) {
    ...
    const fetchExisting = async () => {
      ...
      try {
        ...
        if (existingData) {
          ...
          const selected = ((existingData.q1 as unknown as string[]) || []).filter((e: string) => allDefinedEmotions.includes(e));
          const free = ((existingData.q1 as unknown as string[]) || []).find((e: string) => !allDefinedEmotions.includes(e)) || '';

          let parsedCustomAnswers: Record<string, any> = {};
          if ('customAnswers' in existingData && existingData.customAnswers) {
            try {
              parsedCustomAnswers = JSON.parse(existingData.customAnswers as string);
            } catch {}
          } else if ('referralSources' in existingData && existingData.referralSources && Array.isArray(existingData.referralSources) && existingData.referralSources.length > 0) {
  ...
  const handlePrevStep = () => {
    ...
    if (step > 1) {
      ...
      if (isEvent) {
        router.push(eventId ? `/events/${eventId}/viewer` : '/viewer');
      } else {
        router.push(eventId ? `/events/${eventId}/survey/exhibits?surveySkipped=true` : `/survey/creators?surveySkipped=true`);
      }
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEvent) {
        if (!eventId) throw new Error('イベントIDが見つかりません');
        ...
        if (editId) {
          const { error } = await saveEventFeedback(payload, editId);
          if (error) throw error;
        } else {
          payload.id = crypto.randomUUID();
          payload.eventId = eventId;
          const { error } = await saveEventFeedback(payload, null);
          if (error) throw error;
        }

        if (settings.hasExhibits) {
          router.push(eventId ? `/events/${eventId}/survey/exhibits` : '/survey/creators');
        } else {
        ...
        if (exhibitId) {
          ...
          if (editId) {
            const { error } = await saveExhibitFeedback(payload, editId);
            if (error) throw error;
          } else {
            payload.id = crypto.randomUUID();
            payload.exhibitId = exhibitId;
            const { error } = await saveExhibitFeedback(payload, null);
            if (error) throw error;
          }
      ...
      try {
        ...
        const res = await fetch('/api/ai/generate', {
          ...
          body: JSON.stringify({
            ...
            q2: tQ2,
            q3: tQ3,
            targetName: isEvent ? (settings.eventName || 'イベント') : (exhibit?.name || '展示作品'),
            targetType: isEvent ? 'event' : 'exhibit',
          }),
        });
    ...
    return (
      <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div className="card" style={{ marginBottom: '32px' }}>
          ...
          {isEvent && Object.keys(customAnswers).length > 0 && (
            <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid var(--color-border)' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {customQuestionsList.filter((q: any) => customAnswers[q.id]).map((q: any) => (
                  <div key={q.id}>
                    <div style={{ fontSize: '0.85rem', color: 'var(--color-text-light)' }}>Q. {q.label}</div>
                    <div style={{ fontWeight: 500 }}>
                      {Array.isArray(customAnswers[q.id]) 
                        ? (customAnswers[q.id] as string[]).join('、') 
                        : customAnswers[q.id]}
                    </div>
                  </div>
                ))}
